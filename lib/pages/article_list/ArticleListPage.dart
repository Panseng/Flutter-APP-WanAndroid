...
import 'package:flutter_app/model/article_list/ArticleListModel.dart';
import 'package:flutter_app/pages/article_list/ArticleItemPage.dart';
...
class ArticleListPage extends StatefulWidget {
	...
	ArticleListPage({Key key, this.header, @required this.request, this.emptyMsg,
		this.selfControl = true, this.showQuickTop,  this.keepAlive = false});
...
class ArticleListPageState extends State<ArticleListPage>
		...
		if (itemCount <= 0) {
			return EmptyHolder(
				msg: (widget.emptyMsg == null)
						? (_havaMoreData ? "Loading" : "Not Found")
						: widget.emptyMsg,
			);
		}
	...
	Future<Null> _loadListData(int page) {
		...
		return widget.request(page).then((response) {
			...
			_havaMoreData = originListLength != _listData.length;
		});
	}
